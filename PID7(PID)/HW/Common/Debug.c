...
int mqtt_value = 0;                         // 存储解析的 value 值
/**
 * @brief  向 UART1 发送数据
 */
...
void Extract_Value(char *data) {
    char *start = strstr(data, "receive MQTT");  // 查找帧头
    char *end = strchr(data, '}');  // 查找帧尾
    if (start && end && start < end) {
        char *value_pos = strstr(start, "value=");
        if (value_pos) {
            mqtt_value = atoi(value_pos + 6);  // 提取 value= 后的数字
            printf(" value: %d\r\n", mqtt_value);
        }
