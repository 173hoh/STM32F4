...
void OLED_Showshuzhi(int shuzhi1,int shuzhi2)
{
	if(shuzhi1<0)
	{
		OLED_ShowStr(80,2,"-",2);		//显示负号
		shuzhi1=-shuzhi1;	
	}
	else OLED_ShowStr(80,2," ",2); //掩盖负号
	OLED_Shownum1(88,2,shuzhi1,2);	    //显示俯仰角
	
	if(shuzhi2<0)
	{
		OLED_ShowStr(80,4,"-",2);		//显示负号
		shuzhi2=-shuzhi2;	
	}
	else OLED_ShowStr(80,4," ",2); //掩盖负号
	OLED_Shownum1(88,4,shuzhi2,2);	    //显示俯仰角
}
...
void OLED_Init(void)
{
	...
	OLED_ShowStr(6,1,"By: wujundong",2);
	delay_ms(1000);
	OLED_ShowStr(6,3,"QQ: 1724268144",2);
	delay_ms(1000);
	OLED_ShowStr(10,5,"  2022/9/22",2);
	delay_ms(1000);
	OLED_Fill(0x00);
