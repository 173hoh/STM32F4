...
float Vertical_Kp=-1080;//直立环KP、KD
float	Vertical_Kd=0;//-6.3;

float	Velocity_Kp=0;//-0.5;	//速度环KP、KI
float	Velocity_Ki=0;//.0025;
...
void EXTI9_5_IRQHandler(void)
{
	if(EXTI_GetITStatus(EXTI_Line5)!=0)//一级判定
	{
		...
		if(PBin(5)==0)//二级判定
		{
			...
			Vertical_out=Vertical(Vertical_out+Med_Angle,Roll,gyrox);				//直立环
