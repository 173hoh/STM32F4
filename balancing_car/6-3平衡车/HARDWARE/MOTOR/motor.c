...
{		 		
   ...
   MiniBalance_Motor_Init();	
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2, ENABLE);	//使能定时器2时钟，定时器2是APB1
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA  | RCC_APB2Periph_AFIO, ENABLE);  //使能GPIO外设和AFIO复用功能模块时钟
   
	//设置该引脚为复用输出功能,输出TIM2 CH1 CH1的PWM脉冲波形
...
void Set_Pwm(int moto1,int moto2)
{
	if(stop_flag==0)
	{
    	if(moto1>0)			AIN2=0,			AIN1=1;//AIN2 PAout(2);AIN1 PAout(3)
			else 	          AIN2=1,			AIN1=0;
			PWMA=myabs(moto1);
		  if(moto2>0)	BIN1=0,			BIN2=1;//BIN1 PAout(4);BIN2 PAout(5)
			else        BIN1=1,			BIN2=0;
			PWMB=myabs(moto2);	
	}
