...
u8 My_RTC_Init(void)
{
    ...
    if (RTC_ReadBackupRegister(RTC_BKP_DR0) != 0x5050) // 是否第一次配置?
    {
        ...
        // 替代 RCC_GetRTCClockSource
        if (((RCC->BDCR & RCC_BDCR_RTCSEL) >> 8) != 0x01) {
            return 2; // RTC 时钟源设置失败
        }

        RTC_InitStructure.RTC_AsynchPrediv = 127; // RTC异步分频系数
        RTC_InitStructure.RTC_SynchPrediv = 255;  // RTC同步分频系数
        ...
        RTC_WriteBackupRegister(RTC_BKP_DR0, 0x5050); // 标记已初始化
    }
