 ...
 */
void bsp_timer5_init(uint16_t period_us, timer5_update_callback callback)
{
	...
	nvic_irq_enable(TIMER5_DAC_IRQn, 1, 2);                     // TIMER 中断设置，抢占优先级，子优先级
...
/**
 * @brief   TIMER5 定时器解除初始化，停止中断
 *
 */
void bsp_timer5_deinit(void)
{
...
/**
 * @brief   配置 TIMER6 定时器 (每100微秒计数一次)
 *
 * @param   period_100us: 设置自动重装载寄存器的值，每个计数表示100微秒
 * @param       callback: TIMER6 计数更新时的回调函数
 */
void bsp_timer6_init(uint16_t period_100us, timer6_update_callback callback)
{
    ...
    timer_initpara.alignedmode       = TIMER_COUNTER_EDGE;      // 对齐模式
    timer_initpara.counterdirection  = TIMER_COUNTER_UP;        // 向上计数模式
    timer_initpara.period            = period_100us - 1;        // 自动重装载寄存器周期的值(计数值)
    timer_initpara.clockdivision     = TIMER_CKDIV_DIV1;        // 采样分频

    timer_init(TIMER6, &timer_initpara);

	nvic_irq_enable(TIMER6_IRQn, 1, 2);                         // TIMER 中断设置，抢占优先级，子优先级
...
/**
 * @brief   TIMER6 定时器解除初始化，停止中断
 *
 */
void bsp_timer6_deinit(void)
{
