...
void bsp_usart_deinit(void)
{
    ...
    usart_disable(BSP_USART);
    usart_deinit(BSP_USART);
    rcu_periph_clock_disable(BSP_USART_TX_RCU);
    rcu_periph_clock_disable(BSP_USART_RX_RCU);
    rcu_periph_clock_disable(BSP_USART_RCU);
}
...
void BSP_USART_IRQHandler(void)
{
    if(usart_interrupt_flag_get(BSP_USART, USART_INT_FLAG_RBNE) == SET) {       // 接收缓冲区不为空
        ...
        if (bsp_usart_recv_count < BSP_USART_RECEIVE_LENGTH) {                  // 缓存区未满则缓存接收到的数据
            bsp_usart_recv_buffer[bsp_usart_recv_count++] = byte;
        }
    }

    if(usart_interrupt_flag_get(BSP_USART, USART_INT_FLAG_IDLE) == SET) {       // 检测到帧中断
        usart_data_receive(BSP_USART);                                          // 接收数据，用于清除 IDLE 标志位
    }
