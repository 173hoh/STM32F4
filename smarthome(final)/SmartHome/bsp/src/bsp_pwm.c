...
void bsp_timer0_pwm_init(uint32_t period_us, uint32_t duty, uint16_t timer_channel, timer0_update_callback callback)
{
    ...
    if (callback) {
        nvic_irq_enable(TIMER0_Channel_IRQn, 2, 2);                     // TIMER 中断设置，抢占优先级，子优先级
...
/**
 * @brief   TIMER0 PWM 解除初始化
 *
 */
void bsp_timer0_pwm_deinit(void)
{
...
void bsp_timer2_pwm_init(uint32_t period_us, uint32_t duty, uint16_t timer_channel, timer2_update_callback callback)
{
    ...
    if (callback) {
        nvic_irq_enable(TIMER2_IRQn, 2, 2);                             // TIMER 中断设置，抢占优先级，子优先级
...
/**
 * @brief   TIMER2 PWM 解除初始化
 *
 */
void bsp_timer2_pwm_deinit(void)
{
