...
void key037_interrupt_init(void)
{
    ...
    nvic_irq_enable(KYO37_EXTI_IRQ, 2U, 2U);
 ...
 *
 */
void ky037_deinit(void)
{
    exti_interrupt_disable(KYO37_EXIT_X);
    nvic_irq_disable(KYO37_EXTI_IRQ);
    rcu_periph_clock_disable(KY037_DI_RCU);

    adc_disable(KY037_ADC);
    rcu_periph_clock_disable(KY037_ADC_RCU);

    rcu_periph_clock_disable(KY037_AI_RCU);
}
