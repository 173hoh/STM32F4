...
void su03t_init(uint32_t band_rate, su03t_recv_callback callback)
{
    ...
    if (callback) {
        ...
        nvic_irq_enable(SU03T_UART_IRQ, 2, 2);                      // 配置中断优先级
 ...
 *
 */
void su03t_deinit(void)
{
    nvic_irq_disable(SU03T_UART_IRQ);
    usart_disable(SU03T_UART);
    usart_deinit(SU03T_UART);
    rcu_periph_clock_disable(SU03T_TX_RCU);
    rcu_periph_clock_disable(SU03T_RX_RCU);
    rcu_periph_clock_disable(SU03T_UART_RCU);
}
