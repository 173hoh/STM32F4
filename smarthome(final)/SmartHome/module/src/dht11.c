...
void dht11_init(void)
{
    ...
    gpio_bit_set(DHT11_PORT, DHT11_PIN);
}
...
{
    rcu_periph_clock_disable(DHT11_RCU);
}
...
void dht11_start(void)
{
    ...
    DHT11_DA_OUT(RESET);                                // 将数据线拉低至少18ms
    delay_1ms(19);

    DHT11_DA_OUT(SET);                                  // 释放数据线，等待DHT11响应
...
uint8_t dht11_check_response(void)
{
    ...
    while (DHT11_DA_IN == RESET && --timeout > 0);      // 等待高电平的到来
    delay_1us(40);                                      // 等待高电平结束

    while (DHT11_DA_IN == SET && --timeout > 0);
...
uint8_t dht11_read_byte(void)
{
    ...
    for (uint8_t i = 0; i < 8; i++) {
        while (DHT11_DA_IN == RESET);                  // 等待低电平时隙过去

        delay_1us(30);

        if (DHT11_DA_IN) {                             // 等待了30us后如果还没跳转低电平，则收到的数据位为1
            data |= 1 << (7 - i);
        }

        while (DHT11_DA_IN == SET);                    // 等待高电平过去，开始下一bit
    }
...
void dht11_read_data(float *temperature, float *humidity)
{
    ...
    if (dht11_check_response()) {                       // 检查 DHT11 响应
        ...
        *temperature = data[2] + data[3] * .1f;
        ...
        printf("DHT11 is not responding");
    }
}
