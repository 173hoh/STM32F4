...
void mq2_interrupt_init(void)
{
    ...
    nvic_irq_enable(MQ2_EXTI_IRQ, 2U, 2U);
...
{
    exti_interrupt_disable(MQ2_EXIT_X);
    nvic_irq_disable(MQ2_EXTI_IRQ);
    rcu_periph_clock_disable(MQ2_DI_RCU);

    adc_disable(MQ2_ADC);
    rcu_periph_clock_disable(MQ2_ADC_RCU);
    rcu_periph_clock_disable(MQ2_AI_RCU);
}
