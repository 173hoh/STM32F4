...
void dwin_init(dwin_recv_callback callback)
{
    ...
    if (callback) {
        ...
        nvic_irq_enable(DWIN_USART_IRQn, 1, 2);                     // 配置中断优先级
...
{
    nvic_irq_disable(DWIN_USART_IRQn);
    usart_disable(DWIN_USART);
    usart_deinit(DWIN_USART);
    rcu_periph_clock_disable(DWIN_USART_RCU);
    rcu_periph_clock_disable(DWIN_TX_RCU);
    rcu_periph_clock_disable(DWIN_RX_RCU);
}
