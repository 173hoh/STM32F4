...
void beep_deinit(void)
{
  ...
  timer_deinit(BEEP_TIMER);
  rcu_periph_clock_disable(BEEP_TIMER_RCU);
  rcu_periph_clock_disable(BEEP_RCU);
}
...
/**
  * @brief  蜂鸣器鸣叫函数
  * @param  num：蜂鸣器鸣叫次数
  * @param  enable_time：蜂鸣器鸣叫时长，单位为毫秒
  * @param  disable_time：蜂鸣器鸣叫间隙时长，单位为毫秒
  * @retval 无
  */
void beep_beep(uint16_t num,uint16_t enable_time,uint16_t disable_time)
{
    uint16_t i = 0;
    for(;i<num;i++){
        beep_enable();
