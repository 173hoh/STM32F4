...
unsigned char card_0[4]= {235,21,44,131};
unsigned char card_1[4]= {22,9,151,243};
...
unsigned char blue_card[4]= {235,21,44,131};
unsigned char white_card[4]= {22,9,151,243};
...
#define MAX_SAVED_IDS 100  // 最多缓存的 ID 数量
#define   RC522_DELAY()  delay_us( 20 )
u8 savedIDs[MAX_SAVED_IDS][9]; // 缓存已存储的 ID
u8 savedIDCount = 0;          // 已存储的 ID 数量
...
{ 
    ...
    status = PcdAnticoll(SN);//防冲撞 
		sprintf ( Str1, "card_type: %02X%02X",CT [ 0 ],CT [ 1 ]);
	//printf ( "%s\r\n",Str1); 
	if(status == MI_OK)
	{ 
		//防冲撞成功
		...
		sprintf ( Str2, "The Card ID is: %02X%02X%02X%02X",SN[ 0 ],SN [ 1 ],SN [ 2 ],SN [ 3 ] );
		//printf ( "%s\r\n",Str2); 
	}
	status =PcdSelect(SN);
	}
...
//    {
//        if (strcmp((char *)savedIDs[i], (char *)id) == 0)
//        {
//            return 1; // ID 已存在
//        }
//    }
...
//void SaveIDToSD(u8 *id)
//{
//    // 检查是否是重复 ID
//    if (IsIDSaved(id))
//    {
//        printf("Duplicate ID: %s, not saved.\r\n", id);
//        return;
//    }

//    // 打开文件（如果存在则追加，不存在则创建）
//    ret = f_open(&fil, "0:/id_log.txt", FA_OPEN_EXISTING | FA_WRITE);
//    if (ret == FR_NO_FILE)
//    {
//        ret = f_open(&fil, "0:/id_log.txt", FA_CREATE_NEW | FA_WRITE);
//    }
//    if (ret == FR_OK)
//    {
//        // 移动到文件末尾
...
//        // 写入 ID
//        f_write(&fil, id, strlen((char *)id), &bww);
...
//        // 将新 ID 添加到缓存
//        if (savedIDCount < MAX_SAVED_IDS)
//        {
//            strcpy((char *)savedIDs[savedIDCount], (char *)id);
//            savedIDCount++;
//        }
//        else
//        {
//            printf("ID cache is full, new IDs will not be cached.\r\n");
//        }
