...
/*-----------------------------------------------------------*/
	...
	BaseType_t xReturn;
		...
		#if( portSTACK_GROWTH > 0 )
		{
			/* 堆向上生长，栈向下生长Allocate space for the TCB.  Where the memory comes from depends on
			...
			if( pxNewTCB != NULL )
			{
				...
				be deleted later if required.先申请任务堆栈内存  */
		...
		#else /* portSTACK_GROWTH <0，先申请栈，再申请任务控制块*/
		{
		StackType_t *pxStack;

			/* Allocate space for the stack used by the task being created. 先申请任务控制块*/
			...
			if( pxStack != NULL )
			{
				...
				if( pxNewTCB != NULL )
				{
					/* Store the stack location in the TCB. 将前面申请到的堆栈地址赋值给控制块的堆栈成员*/
...
UBaseType_t x;
	...
	{
		/* Fill the stack with a known value to assist debugging. 全部初始化为一个数，可以检查堆栈空间*/
	...
	by the port. STM32就是向下生长的*/
	#if( portSTACK_GROWTH < 0 )
	{
		/*申请的是堆，堆从下往上，ulStackDepth是整个堆栈大小-1刚好是栈顶*/
	...
	#endif /* configUSE_MUTEXES 初始化列表项*/

	vListInitialiseItem( &( pxNewTCB->xStateListItem ) );
	vListInitialiseItem( &( pxNewTCB->xEventListItem ) );

	/* Set the pxNewTCB as a link back from the ListItem_t.  This is so we can get
	back to	the containing TCB from a generic item in a list.监测任务控制块状态 */
	...
	#else /* portUSING_MPU_WRAPPERS初始化任务堆栈，保存上下文信息，方便后期任务切换使用 */
	{
...
static void prvAddNewTaskToReadyList( TCB_t *pxNewTCB )
{
	...
	taskENTER_CRITICAL();
	{
		...
		if( pxCurrentTCB == NULL )
		{
			...
			the suspended state - make this the current task.pxCurrentTCB指向优先级最高的任务 */
			...
			if( uxCurrentNumberOfTasks == ( UBaseType_t ) 1 )
			{
				...
				fails, but we will report the failure.初始化列表 */
...
/*-----------------------------------------------------------*/
	...
	TCB_t *pxTCB;
		...
		taskENTER_CRITICAL();
		{
			...
			being suspended.挂起句柄 */
			pxTCB = prvGetTCBFromHandle( xTaskToSuspend );

			traceTASK_SUSPEND( pxTCB );

			/* Remove task from the ready/delayed list and place in the
			suspended list.从相应状态列表中移除 */
			if( uxListRemove( &( pxTCB->xStateListItem ) ) == ( UBaseType_t ) 0 )
			{
				taskRESET_READY_PRIORITY( pxTCB->uxPriority );
			}
			...
			/* Is the task waiting on an event also? 从事件列表项移除*/
			if( listLIST_ITEM_CONTAINER( &( pxTCB->xEventListItem ) ) != NULL )
			{
				( void ) uxListRemove( &( pxTCB->xEventListItem ) );
			}
...
/*-----------------------------------------------------------*/
	...
	TCB_t * const pxTCB = ( TCB_t * ) xTaskToResume;
		...
		currently executing task.挂起完的任务丢进就绪列表里  */
...
/*-----------------------------------------------------------*/
	...
	UBaseType_t uxSavedInterruptStatus;
		...
		provided on the following link:
		http://www.freertos.org/RTOS-Cortex-M3-M4.html 
		portASSERT_IF_INTERRUPT_PRIORITY_INVALID();检测调用的api函数是否在管理范围内
		还有是否全部设置为抢占优先级位
		*/
		portASSERT_IF_INTERRUPT_PRIORITY_INVALID();
